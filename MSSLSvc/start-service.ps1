$srvName = "Enterprise Library Semantic Logging Service"

$servicePrior = Get-Service | Where-Object {$_.Name -eq $srvName}

if($servicePrior -ne $null)
{
    Write-Host "<$srvName> is now " $servicePrior.status
...
{
    Write-Error "Service <$srvName> is not installed"
}
